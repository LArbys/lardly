...
from lardly.data.larlite_pcaxis import visualize_event_pcaxis
...
class PCAxisPlotter(BasePlotter):
    """
    Plotter for cluster line segments represented as PC-axis
    
    This plotter visualizes clusters saved larlite::pcaxis objects.
    """
    
    def __init__(self):
        """Initialize the PCAxis plotter"""
        super().__init__("PCAxis", "PCAxis")
        self._imports_successful = _IMPORTS_SUCCESSFUL
        self._input_trees_present = []
        logger.info(f"PCAxisPlotter initialized. Imports successful: {self._imports_successful}")
    ...
    def is_applicable(self, tree_keys: List[str]) -> bool:
        ...
        if not self._imports_successful:
            logger.warning("PCAxisPlotter not applicable: imports failed")
            return False
            
        # Check for input cluster trees
        self._input_trees_present = []
        for tree_key in tree_keys:
            treename_components = tree_key.split("_")
            if len(treename_components)>0 and treename_components[0]=="pcaxis":
                logger.info(f"Found input tree: {tree_key}")
                self._input_trees_present = [tree_key]
        ...
        if len(self._input_trees_present)==0:
            logger.info("PCAxisPlotter not applicable: no input trees found")
            return False
        
        logger.info(f"PCAxisPlotter is applicable, found trees: {self._input_trees_present}")
        return True
    ...
    def make_traces(self, tree_dict: Dict[str, Any], options: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Get cluster data
            traces = []
            producer_name = cluster_source[len("pcaxis_"):-len("_tree")]
            self.log_info("getting pcaxis from source: {cluster_source} --> {producer_name}")
            ev_cluster = iolarlite.get_data("pcaxis", producer_name)
            self.log_info(f"Number of axes from {cluster_source}: {ev_cluster.size()}")

            traces = visualize_event_pcaxis( ev_cluster )
    ...
    def register_callbacks(self, app):
        ...
        try:
            ...
            logger.info("PCAxisPlotter callbacks registered successfully")
        except Exception as e:
            logger.error(f"Error registering PCAxisPlotter callbacks: {e}")
            import traceback
            logger.error(traceback.format_exc())
